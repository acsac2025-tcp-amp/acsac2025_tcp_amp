    ...
    pac = bytes(
        Ether(
            bytes(
                ...
                sender.eth_header/IP(src=sender.src_ip,dst=dst_ip)/TCP(sport=sender.src_port,dport=dport,flags='S',seq=seq,ack=ack,window=64240,options=[('MSS',1460)]) # large mss or default
            )
        )
    )
    return pac
            ...
            bytes(
                sender.eth_header/IP(
                    src=sender.src_ip,dst=dst_ip
                        ...
                        sport=sender.src_port,dport=dport,flags='A',seq=seq,ack=ack,window=64240
                    )
            )
        )   
    )
            ...
            bytes(
                    sender.eth_header/IP(
                        src=sender.src_ip,dst=dst_ip
                            ...
                            sport=sender.src_port,dport=dport,flags='PA',seq=seq,ack=ack,window=64240
                            )/payload_template
                        )
                )
        )
